...
def split_text(text: str, target_len: int = 30) -> list:
    """按标点切分文本，每段长度尽量接近 target_len（默认参考音频文本量）"""
    max_len = max(target_len * 2, 60)  # 单段最长不超过 target_len 的 2 倍
    min_len = max(target_len // 2, 8)  # 过短则合并
        ...
        if len(s) <= max_len:
            ...
            for p in sub:
                if len(buf) + len(p) <= max_len:
                    ...
                    buf = p
            if buf.strip(): refined.append(buf.strip())
...
def save_audio(speech: torch.Tensor, sample_rate: int, filename: str) -> str:
    ...
    # 响度归一化到 -16 LUFS（视频配音标准）
    try:
        import subprocess
        normalized_path = output_path.with_suffix('.norm.wav')
    ...
    try:
        ...
        def run_inference(seg_text):
            if mode == "sft":
                ...
                return model.inference_zero_shot(seg_text, prompt_text, prompt_audio, stream=stream, speed=speed)
            elif mode == "cross_lingual":
                return model.inference_cross_lingual(seg_text, prompt_audio, stream=stream, speed=speed)
            elif mode == "instruct":
                if hasattr(model, 'inference_instruct2'):
                    return model.inference_instruct2(seg_text, instruct_text, prompt_audio, stream=stream, speed=speed)
                else:
                    return model.inference_instruct(seg_text, spk_id, instruct_text, stream=stream, speed=speed)
            else:
                raise HTTPException(400, f"Unknown mode: {mode}")

        # 文本切分：以参考音频文本长度为目标段长，短文本不切分
        ref_len = len(re.sub(r'[^\u4e00-\u9fffA-Za-z0-9]', '', custom_voice["text"] if custom_voice else prompt_text))
        target_len = max(ref_len, 15) if ref_len > 0 else 30
        segments = split_text(text, target_len=target_len) if len(text) > target_len * 1.5 else [text]
        print(f"Text split into {len(segments)} segment(s)")

        if stream:
            def multi_segment_stream():
                for i, seg in enumerate(segments):
                    cleanup = (prompt_audio if is_temp_file else None) if i == len(segments) - 1 else None
                    yield from generate_audio_stream(run_inference(seg), model.sample_rate, cleanup_path=cleanup)
            return StreamingResponse(
                multi_segment_stream(),
                media_type="audio/pcm",
        ...
        all_speeches = []
        for seg in segments:
            for chunk in run_inference(seg):
                all_speeches.append(chunk['tts_speech'])

        full_speech = torch.cat(all_speeches, dim=1)
...
async def audio_trim(
    ...
    """裁剪音频片段，start/end 单位为秒，end=0 表示到结尾"""
    import subprocess
    content = await audio.read()
    src = INPUT_DIR / f"trim_src_{uuid.uuid4().hex}.wav"
    dst = PROCESSED_DIR / f"trim_{uuid.uuid4().hex}.wav"
    ...
    try:
        ...
        fade_out_st = max(0, duration - 0.05) if duration > 0.1 else 0
        cmd += ["-af", f"afade=t=in:st=0:d=0.05,afade=t=out:st={fade_out_st:.2f}:d=0.05", str(dst)]
        result = subprocess.run(cmd, capture_output=True, timeout=60)
        if result.returncode != 0:
            raise HTTPException(500, "ffmpeg error: " + result.stderr.decode())
        return FileResponse(str(dst), media_type="audio/wav", filename=dst.name)
    finally:
        if src.exists(): src.unlink()

@app.post("/api/audio/separate")
async def audio_separate(audio: UploadFile = File(...)):
    """人声分离（demucs htdemucs），返回人声音频"""
    import subprocess
    content = await audio.read()
    job_id = uuid.uuid4().hex
    src = INPUT_DIR / f"sep_src_{job_id}.wav"
    out_dir = PROCESSED_DIR / f"sep_{job_id}"
    src.write_bytes(content)
    try:
        result = subprocess.run(
        ...
        if result.returncode != 0:
            raise HTTPException(500, "demucs error: " + result.stderr.decode()[-500:])
        vocals = out_dir / "htdemucs" / src.stem / "vocals.wav"
        if not vocals.exists():
            raise HTTPException(500, "人声文件未生成")
        dst = PROCESSED_DIR / f"vocals_{job_id}.wav"
        import shutil
        shutil.copy(str(vocals), str(dst))
        return FileResponse(str(dst), media_type="audio/wav", filename=dst.name)
    finally:
        if src.exists(): src.unlink()
        if out_dir.exists():
...
async def audio_info_get(path: str):
    """获取音频时长（秒）"""
    import subprocess
    result = subprocess.run(
    ...
    if result.returncode != 0:
        raise HTTPException(400, "无法读取音频信息")
    return {"duration": float(result.stdout.decode().strip())}

@app.post("/api/audio/info")
async def audio_info_post(audio: UploadFile = File(...)):
    """获取上传音频的时长（秒）"""
    import subprocess
    content = await audio.read()
    src = INPUT_DIR / f"info_{uuid.uuid4().hex}.wav"
    src.write_bytes(content)
    try:
        result = subprocess.run(
            ["ffprobe", "-i", str(src), "-show_entries", "format=duration",
             "-v", "quiet", "-of", "csv=p=0"],
            capture_output=True, timeout=10
        )
...
async def audio_normalize(audio: UploadFile = File(...), lufs: float = Form(-16.0)):
    """响度归一化，默认 -16 LUFS（视频配音标准）"""
    import subprocess
    content = await audio.read()
    job_id = uuid.uuid4().hex
    src = INPUT_DIR / f"norm_src_{job_id}.wav"
    dst = PROCESSED_DIR / f"norm_{job_id}.wav"
    src.write_bytes(content)
    try:
        result = subprocess.run(
            ['ffmpeg', '-y', '-i', str(src),
        ...
        if result.returncode != 0:
            raise HTTPException(500, 'loudnorm failed: ' + result.stderr.decode()[-300:])
        return FileResponse(str(dst), media_type='audio/wav', filename=f'normalized_{lufs}lufs.wav')
    finally:
        if src.exists(): src.unlink()
...
# (HTML moved to static/index.html)
_UNUSED = '''
<!DOCTYPE html>
<html lang="zh-CN">
<head>
